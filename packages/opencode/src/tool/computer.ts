...
// --- open: launch apps, files, folders, URLs ---

const OpenSchema = Schema.Struct({
  target: Schema.String.annotations({ description: "File path, folder, URL, or application name to open (e.g. 'notepad', 'https://...', 'C:\\Users\\...', 'README.md')" }),
})
...
export const ComputerOpenTool = Tool.define(
  ...
  Effect.gen(function* () {
    return {
      ...
      execute: (input) =>
        Effect.gen(function* () {
          const safe = input.target.replace(/"/g, '\\"')
          ...
          return { title: "Opened", metadata: {}, output: `Opened: ${input.target}` }
        }),
    } satisfies Tool.DefWithoutID
  }),
)

// --- process: list, kill, inspect ---

const ProcessActionSchema = Schema.Literal("list", "kill", "info")

const ProcessSchema = Schema.Struct({
  action: ProcessActionSchema.annotations({ description: "list → running processes. kill → terminate by name/PID. info → detailed info by name/PID." }),
  name: Schema.optional(Schema.String.annotations({ description: "Process name (e.g. 'chrome', 'notepad') or PID" })),
})
      ...
      execute: (input) =>
        Effect.gen(function* () {
          ...
          if (input.action === "kill") {
            if (!input.name) return { title: "Error", metadata: {}, output: "name required" }
            if (process.platform === "win32") {
              yield* pwsh(`Stop-Process -Name "${input.name}" -Force -ErrorAction SilentlyContinue`)
              yield* pwsh(`Stop-Process -Id "${input.name}" -Force -ErrorAction SilentlyContinue`)
            } else {
              yield* run(`kill -9 ${input.name} 2>/dev/null ; pkill -9 -f "${input.name}" 2>/dev/null`, 5_000)
            }
            return { title: "Killed", metadata: {}, output: `Killed: ${input.name}` }
          }
          if (input.action === "info") {
            if (!input.name) return { title: "Error", metadata: {}, output: "name required" }
            const raw = process.platform === "win32"
              ? yield* pwsh(`Get-Process -Name "${input.name}" -ErrorAction SilentlyContinue | Select-Object * | ConvertTo-Json -Compress`)
              : yield* run(`ps -p ${input.name} -o pid,ppid,pcpu,pmem,rss,comm --no-headers 2>/dev/null || ps aux | grep -v grep | grep "${input.name}"`)
            return { title: "Process info", metadata: {}, output: raw || `Not found: ${input.name}` }
          }
...
const SystemSchema = Schema.Struct({
  category: Schema.Literal("cpu", "memory", "disk", "network", "all").annotations({ description: "Which system info category" }),
})
    ...
    return {
      ...
      execute: (input) =>
        Effect.gen(function* () {
          ...
          if ((input.category === "disk" || input.category === "all") && process.platform === "win32") {
...
// --- desktop: notifications, volume, power actions ---

const DesktopSchema = Schema.Struct({
  action: Schema.Literal("notify", "volume", "lock", "sleep", "shutdown", "restart").annotations({ description: "Desktop action" }),
  message: Schema.optional(Schema.String.annotations({ description: "Notification message" })),
  title: Schema.optional(Schema.String.annotations({ description: "Notification title" })),
  level: Schema.optional(Schema.Number.annotations({ description: "Volume 0-100" })),
})
      ...
      execute: (input) =>
        Effect.gen(function* () {
          if (input.action === "notify") {
            const msg = (input.message ?? "Notification from opencode").replace(/"/g, '\\"')
            const ttl = (input.title ?? "opencode").replace(/"/g, '\\"')
            if (process.platform === "win32") {
          ...
          if (input.action === "volume") {
            if (process.platform === "win32") {
              ...
              yield* run(`pactl set-sink-volume @DEFAULT_SINK@ ${input.level ?? 50}% 2>/dev/null`, 5_000)
            }
            return { title: "Volume", metadata: {}, output: `Volume: ${input.level ?? 50}` }
          }
          ...
          return { title: "Invalid", metadata: {}, output: `Unknown: ${input.action}` }
        }),
    } satisfies Tool.DefWithoutID
  }),
)
